...
void SkyClass::Draw(ID3D11DeviceContext1 * mImmediateContext, std::shared_ptr<CameraClass> Camera, DirectionalLight & light)
{
	...
	mImmediateContext->PSSetShaderResources(0, 1, newInscatterText->GetAddressOfSRV());
	mImmediateContext->PSSetShaderResources(1, 1, newTransmittanceText->GetAddressOfSRV());
	mImmediateContext->PSSetShaderResources(2, 1, newIrradainceText->GetAddressOfSRV());
