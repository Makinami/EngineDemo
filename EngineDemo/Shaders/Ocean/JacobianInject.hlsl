...
void main( int3 DTid : SV_DispatchThreadID )
{
	...
	float2 dDdx = 0.5 * fftSize / GRID_SIZE[DTid.z] * (fftWaves[uint3(d_position.z, DTid.yz)] - fftWaves[uint3(d_position.x, DTid.yz)]);
	float2 dDdy = 0.5 * fftSize / GRID_SIZE[DTid.z] * (fftWaves[uint3(DTid.x, d_position.w, DTid.z)] - fftWaves[uint3(DTid.x, d_position.y, DTid.z)]);

	Jacobian = (1.0 + dDdx.x) * (1.0 + dDdy.y) - dDdx.y * dDdy.x;
	float satJacobian = max(k*(-Jacobian + M), 0.0);

	turbulenceUAV[DTid] = float4(Jacobian, (Jacobian-1.0)*exp(1- DTid.z) + 1.0, turbulence.z*exp(-dt/8.0) + satJacobian*dt*exp(2 - DTid.z), 0.0);
}