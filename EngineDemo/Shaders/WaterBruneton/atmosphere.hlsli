...
/*
 * Modified Eric Bruneton's (INRIA) "Precomputed Atmospheric Scattering"
 */

Texture2D<float3> skyMap : register(t100);
Texture3D<float4> inscatterTex : register(t101);
Texture2D<float3> transmittance : register(t102);
Texture2D<float3> deltaE : register(t103);

// MIN MAG MP - LINEAR; UVW - CLAMP; BORDER - 0
SamplerState samLinearClampBorder0 : register(s0);

#define samInscatter samLinearClampBorder0
#define samTransmittance samLinearClampBorder0
#define samIrradiance samLinearClampBorder0

// ANISOTROPIC
SamplerState samSkyMap : register(s1);

// TEMP
static const float hdrExposure = 0.4;


static const float SUN_INTENSITY = 100.0;

static const float3 earthPos = float3(0.0f, 6360010.0f, 0.0f);
...
{
	return getTransmittanceWithShadow(r, muS) * SUN_INTENSITY;
}

// incident sky light at given position, integrated over hemisphere (irradiance)
// r=length(x)
// muS=dot(x,s) / r
float3 skyIrradiance(float r, float muS)
{
	return getIrradiance(r, muS) * SUN_INTENSITY;
}

// scatter sunlight between two points
// camera = observer
...
float3 skyRadiance(float3 camera, float3 viewdir, float3 sundir, out float3 extinction)
{
	...
	if (r < Rt)
	{
		...
		float4 inScatter = getInscatter(r, rMu / r, muS, nu);
		extinction = getTransmittance(r, mu);
...
// camera = observer
// place = point on the ground
// sundir = unit vector towards the sun
// return scattered light and extinction coefficient
float3 inScattering(float3 camera, float3 place, float3 sundir, out float3 extinction)
{
	float3 result;
	float3 viewdir = place - camera;
	...
	if (r <= Rt)
	{
		...
		if (r < Rg + 600.0f)
		{
			// avoid inprecision problems in aerial perspective near ground
			float f = (Rg + 600.0f) / r;
			r = r * f;
			rMu = rMu * f;
			place = place * f;
		...
		float r1 = length(place);
		float rMu1 = dot(place, viewdir);
		float mu1 = rMu1 / r1;
		float muS1 = dot(place, sundir) / r1;

		if (mu > 0.0)
			extinction = min(getTransmittance(r, mu) / getTransmittance(r1, mu1), 1.0f);
		else
			extinction = min(getTransmittance(r1, -mu1) / getTransmittance(r, -mu), 1.0f);

		float4 inScatter0 = getInscatter(r, mu, muS, nu);
		float4 inScatter1 = getInscatter(r1, mu1, muS1, nu);
...
float3 skyRadiance(float2 u)
{
	return skyMap.SampleLevel(samSkyMap, (u * (0.5 / 1.1) + 0.5), 0.0f);
}
