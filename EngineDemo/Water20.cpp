...
bool WaterClass20::Init(ID3D11Device1 * device, ID3D11DeviceContext1 * dc)
{
...
void WaterClass20::Draw(ID3D11DeviceContext1 * mImmediateContext, std::shared_ptr<CameraClass> Camera)
{
...
void WaterClass20::evaluateWavesGPU(float t, ID3D11DeviceContext1 * mImmediateContext)
{
	time += t;
...
void WaterClass20::BuildQuadPatchVB(ID3D11Device1 * device)
{
...
bool WaterClass20::BuildQuadPatchIB(ID3D11Device1 * device)
{
	vector<UINT> indices;
...
bool WaterClass20::CreateInputLayoutAndShaders(ID3D11Device1 * device)
{
	ifstream stream;
	size_t size;
	char* data;
...
bool WaterClass20::CreateInitialDataResource(ID3D11Device1 * device)
{
...
float WaterClass20::PhillipsSpectrum(int n, int m)
{
...
complex<float> WaterClass20::hTilde_0(int n, int m)
{
	float x1, x2, w;
	do {
...
float WaterClass20::Dispersion(int n, int m)
{
